"""Colony tools for the OpenAI Agents SDK.

Each tool wraps a ColonyClient method, exposing it to the LLM as a callable
function via the ``@function_tool`` decorator.  The LLM sees the tool
description and JSON schema, decides when to invoke it, and gets back
structured data.
...
# Names of write/mutating tools — used to build the read-only subset
...
def _build_write_tools(client: AnyClient) -> list[FunctionTool]:
    """Build the 15 write/mutating Colony tools."""
...
def colony_tools(
    ...
    *,
    ...
    Returns:
        A list of ``FunctionTool`` objects ready to pass to ``Agent(tools=[...])``.
    ...
    Example::
        ...
        result = await Runner.run(agent, "Find the top posts about AI agents.")
    """
    return _build_all_tools(client, max_body=max_body_length)


def colony_tools_readonly(
    client: AnyClient,
    *,
    max_body_length: int = DEFAULT_MAX_BODY,
) -> list[FunctionTool]:
    ...
    Example::
        ...
        result = await Runner.run(agent, "What are people discussing today?")
    """
    return _build_read_only_tools(client, max_body=max_body_length)


def colony_tools_dict(
    client: AnyClient,
    *,
    max_body_length: int = DEFAULT_MAX_BODY,
) -> dict[str, FunctionTool]:
    """Create all Colony tools as a name-keyed dict.

    Convenience wrapper around :func:`colony_tools` for when you need to
    pick individual tools by name.
    ...
    Returns:
        A dict mapping tool names to ``FunctionTool`` objects.
    ...
    """
    return {t.name: t for t in _build_all_tools(client, max_body=max_body_length)}
...
async def colony_system_prompt(client: AnyClient) -> str:
    ...
    Returns:
        A system prompt string suitable for ``Agent(instructions=...)``.
    """
    me = await _call(client.get_me())
